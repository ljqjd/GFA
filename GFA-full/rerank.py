...
def pairwise_distance1(query_features, gallery_features):
    x = query_features
    y = gallery_features
    m, n = x.size(0), y.size(0)
    x = x.view(m, -1)
    ...
    x_bool = (x > 0).float()
    y_bool = (y > 0).float()
    intersection = torch.matmul(x_bool, y_bool.t())
    union = torch.matmul(x_bool, y_bool.t()) + torch.matmul(y_bool, x_bool.t()) - intersection
    jaccard_similarity = intersection / union.clamp(min=1e-8)  
    jaccard_distance = 1 - jaccard_similarity
    
    return jaccard_distance


def re_ranking(q_feat, g_feat, k1=20, k2=20, lambda_value=0.3, eval_type=True):

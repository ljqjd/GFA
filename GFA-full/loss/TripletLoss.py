...
class OrthonormalLoss(nn.Module):
    def __init__(self):
        super(OrthonormalLoss, self).__init__()

    def forward(self, w):
        B, K, C = w.shape
        w_norm = self.featureL2Norm(w)
        WWT = torch.matmul(w_norm, w_norm.transpose(1, 2))
        return F.mse_loss(WWT - torch.eye(K).unsqueeze(0).cuda(), torch.zeros(B, K, K).cuda(), reduction='sum') / (K * K)
...
class OriTripletLoss1(nn.Module):
    """Triplet loss with hard positive/negative mining.
    ...
    def __init__(self, margin=0.3):
        super(OriTripletLoss1, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)

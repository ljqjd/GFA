...
class MSEL_Cos(nn.Module):
    # for features after bn
    # 基于余弦相似度的最小二乘损失函数，用于度量特征之间的相似性和差异性。
...
class MSEL_Feat(nn.Module):
    # compute MSEL loss by the distance between sample and center
    # 基于样本与中心之间距离的最小二乘损失函数（Mean Squared Error Loss，MSEL）
    def __init__(self, num_pos):

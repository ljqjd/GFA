...
from model.CMM import CollaborativeMatchingModule
import matplotlib.pyplot as plt
...
parser.add_argument('--model_path', default='/home/jiaqi/Baseline7/save_model/', type=str, help='model save path')
...
parser.add_argument('--batch-size', default=22, type=int, metavar='B', help='training batch size')
...
parser.add_argument('--seed', default=66, type=int, metavar='t', help='random seed')
...
def plot_embedding(X, y, z, title=None):
    ...
    cx, cy = [], []
    plt.figure(figsize=(12, 10), dpi=100)
    plt.gca().set_facecolor((220/255, 220/255, 255/255))
    r = []
    #print(X.shape[0])
    for i in range(X.shape[0]):
        ...
        if z[i] == 1:
            #print(i, y[i])
            plt.scatter(X[i, 0], X[i, 1], s=120, color=color[y[i]], edgecolor='None', marker='o')
        else:
            plt.scatter(X[i, 0], X[i, 1], s=120, color=color[y[i]], edgecolor='None', marker='^')
...
def train(epoch):
    ...
    with torch.no_grad():
      for batch_idx, (input1, input2, _, label1, _,_) in enumerate(trainloader):
          labels = torch.cat((label1, label1), 0)
          z1 = torch.ones(label1.shape)
          z2 = torch.zeros(label1.shape)
          z = torch.cat((z1, z2), 0)
          print(batch_idx)
       ...
       #   out = torch.cat((input1, input2), 0).
          out = net(torch.cat([input1,input2]))
    #      print(out.shape)
    
          out_flattened = out.view(out.size(0), -1)  
          out_np = out_flattened.cpu().numpy()
          tsne = manifold.TSNE(n_components=2, init='pca', random_state=0)
       #   X_tsne = tsne.fit_transform(out.detach().cpu().numpy())
          X_tsne = tsne.fit_transform(out_np)
          plot_embedding(X_tsne, labels, z)
          plt.savefig(osp.join('/home/jiaqi/Baseline5/save_tsne', 'tsne_{}.jpg'.format(batch_idx)))
